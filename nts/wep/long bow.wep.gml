...
#define weapon_name		return is_object(argument0) ? "long edgling gun" : "big bandit gun"
#define weapon_sprt_hud	return sprBanditBossGun

#define weapon_sprt
	{
	if(instance_is(self, Player)){if(is_object(argument0)){if(argument0.frame > 0)
		{return mskNone}}}
	return sprBanditBossGun
	}
...
#define weapon_swap		return is_object(argument0) ? sndSwapBow : sndBigBanditShootLaugh

#define weapon_gold			return false
#define weapon_laser_sight	return false

#define nts_weapon_examine	return	mod_script_call("weapon", "big bandit gun", "nts_weapon_examine")
...
#define scrDrawing
	...
	if(frame >= 15)
		{
		damage += 0.75 * current_time_scale
		draw += 0.1 * current_time_scale
		...
		if(drawing > 0)
			...
			if(frame >= 10)
				...
				if(other.ammo[@3] >= 2)
					...
					with(instance_create(other.x, other.y, HeavyBolt))
						{
						creator = c
						team = c.team
...
with(argument0)
	...
	var lr = sqrt(sqr(argument1.draw)*4 + 25) * 0.5
	var dg = radtodeg(arctan(6/argument1.draw))
	...
	if(ammo[@3] >= 2){draw_sprite_ext(sprHeavyBolt, 1, x-lx*0.2, y-ly*0.2, 1, 1, gunangle, c_white, 1)}
	}
instance_destroy()
